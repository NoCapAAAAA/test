...
class DirectorHomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Get filter options
        grouped_purchases = m.OrderStorage.objects.annotate(year=ExtractYear("created_at")).values("year").order_by(
            "-year").distinct()
        ...
        if year:
            ...
            grouped_purchases = purchases.annotate(month=ExtractMonth("created_at")).values("month").annotate(
                count=Count("id")).values("month", "count").order_by("month")
